...
//UnitTypes.zenith.weapons.get(0).bullet.despawnHit = false; UnitTypes.zenith.weapons.get(1).bullet.despawnHit = false;        
        ...
        if(rad > 0){
            ...
            if(power > 0 && sap){
                ...
                if(smokeSize == 0) smokeSize = Math.max(Mathf.pow(power, 0.65f) - Mathf.pow(power, 0.25f), 5f);
            }
            ...
            if(smokeSize == 0) smokeSize = (Mathf.pow(power > 0 ? (r / 10f) + (power / 6f) : r / 5f, 0.65f)) * m;
        }
